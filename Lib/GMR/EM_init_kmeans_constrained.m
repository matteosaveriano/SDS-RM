function [Priors, Mu, Sigma] = EM_init_kmeans_constrained(Data, nbStates, firtsPt, lastPt)
%
...
[nbVar, nbPts] = size(Data);

[Data_id, Centers] = kmeans(Data(:, firtsPt:lastPt)', nbStates-2,'MaxIter',500);
 

Mu = Centers';
%Mu = [Mu, mean(Data(:,1:10),2), mean(Data(:,end-9:end),2)];

Mu = [Mu, Data(:,1), Data(:,end)];

Data_id = [repmat(nbStates-1, firtsPt-1, 1); Data_id; repmat(nbStates, nbPts-lastPt, 1)];
